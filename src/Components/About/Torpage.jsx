  ...
  useEffect(() => {
    const fetchTermsOfReference = async () => {
      try {
        ...
        const q = query(documentsRef, where('categories', 'array-contains', 'Terms of reference'));
        const snapshot = await getDocs(q);

        const tors = snapshot.docs.map(doc => ({
          id: doc.id,
          ...doc.data(),
          validationStatus: doc.data().validationStatus || "PENDING"
        }));

        const formattedResults = tors
          .map(result => ({
            id: result.id,
            title: result.title,
    ...
    fetchTermsOfReference();
  }, []);

  const renderLatestTOR = () => {
    ...
    return (
      <article className="bg-white/50 backdrop-blur-sm rounded-lg shadow-lg overflow-hidden mb-8">
        <div className="p-6">
          ...
          {user?.role === 'admin' && (
            <SignatoriesList documentId={latest.id} />
          )}
        </div>
      </article>
    );
